...
# Installed Codex plugins can load this through hooks/hooks.json.
...
if [ -n "$STDIN_CWD" ] && [ -d "$STDIN_CWD" ]; then
  ...
  export CODEX_PLUGIN_PROJECT_DIR="${_GIT_ROOT:-$STDIN_CWD}"
fi
...
# Pass HOOK_PHASE via env var since not every hook payload includes an event name.
...
try:
    ...
    # Older hook payloads may not include a "hook_type" field,
    # so we rely on the shell argument ("pre" or "post") instead.
    hook_phase = os.environ.get("HOOK_PHASE", "post")
    event = "tool_start" if hook_phase == "pre" else "tool_complete"

    # Extract fields from Codex and legacy hook payloads.
