...
Agents are discovered via file globs, merged and deduplicated by agent name:

1. **Plugin personas** — read from `${PLUGIN_ROOT}/agents/**/*.md` and `${PLUGIN_ROOT}/agents/*.md` when `PLUGIN_ROOT` is available.
2. **Project personas** — read from:
   - `./agents/**/*.md` + `./agents/*.md` — project-local agents
3. **User personas** — read from:
   - `~/.codex/agents/**/*.md` + `~/.codex/agents/*.md` — global user agents

Earlier sources take precedence when names collide: user agents > project agents > plugin agents. A custom path can be used instead if the user specifies one.

## How It Works

### Step 1: Discover Available Agents

Search the configured agent markdown paths and parse each file:

- **Plugin agents** come from `${PLUGIN_ROOT}/agents/` when available.
- **Project agents** come from `./agents/`.
- **User agents** come from `~/.codex/agents/`.
- **Built-in Codex subagents** are not enumerated by this skill; use Codex's native subagent tools directly when the session exposes them.
...
If no agents are found after probing file locations, inform the user: "No agent persona files found in the plugin, project, or ~/.codex/agents paths." Then stop.
...
- **Parallel Agent calls, not TeamCreate.** This skill uses parallel Agent tool calls for independent work. TeamCreate (a multi-agent dialogue tool) is only needed when agents must debate or respond to each other.
