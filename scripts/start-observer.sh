...
# and creates instincts. Uses Flash model for cost efficiency.
#
...
CONFIG_DIR="${CODEX_PLUGIN_HOME:-${HOME}/.codex/codex-plugin}/continuous-learning"
...
OBSERVER_INTERVAL_MINUTES="${CODEX_PLUGIN_OBSERVER_RUN_INTERVAL:-${ANTIGRAVITY_LEARNING_RUN_INTERVAL:-5}}"
MIN_OBSERVATIONS="${CODEX_PLUGIN_OBSERVER_MIN_OBSERVATIONS:-${ANTIGRAVITY_LEARNING_MIN_OBSERVATIONS:-20}}"
OBSERVER_ENABLED="${CODEX_PLUGIN_OBSERVER_ENABLED:-${ANTIGRAVITY_LEARNING_OBSERVER_ENABLED:-false}}"
...
case "$ACTION" in
  ...
  start)
    ...
    if [ "$OBSERVER_ENABLED" != "true" ]; then
      echo "Observer is disabled."
      echo "Set CODEX_PLUGIN_OBSERVER_ENABLED=true to enable."
      exit 1
    fi
