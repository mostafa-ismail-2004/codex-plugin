...
def project_tree(tmp_path):
    ...
    continuous_learning = tmp_path / ".codex" / "codex-plugin" / "continuous-learning"
    projects_dir = continuous_learning / "projects"
    global_personal = continuous_learning / "instincts" / "personal"
    global_inherited = continuous_learning / "instincts" / "inherited"
    global_evolved = continuous_learning / "evolved"
    ...
    return {
        ...
        "registry_file": continuous_learning / "projects.json",
    }
...
def patch_globals(project_tree, monkeypatch):
    ...
    monkeypatch.setattr(_mod, "HOMUNCULUS_DIR", project_tree["continuous-learning"])
    ...
    monkeypatch.setattr(_mod, "GLOBAL_OBSERVATIONS_FILE", project_tree["continuous-learning"] / "observations.jsonl")
    return project_tree
...
def test_detect_project_global_fallback(patch_globals, monkeypatch):
    ...
    monkeypatch.delenv("CODEX_PLUGIN_PROJECT_DIR", raising=False)
...
def test_detect_project_from_env(patch_globals, monkeypatch, tmp_path):
    """CODEX_PLUGIN_PROJECT_DIR env var should be used as project root."""
    fake_repo = tmp_path / "my-repo"
    fake_repo.mkdir()
    monkeypatch.setenv("CODEX_PLUGIN_PROJECT_DIR", str(fake_repo))
